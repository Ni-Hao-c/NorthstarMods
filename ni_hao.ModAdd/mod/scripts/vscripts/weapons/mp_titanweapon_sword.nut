...
void function MpTitanWeaponSword_Init()
{
	...
	PrecacheParticleSystem( SWORD_GLOW_PRIME )
		...
		AddDamageCallbackSourceID( eDamageSourceId.melee_titan_punch_fighter, punch_fighter )
	#endif
}
...
void function Sword_DamagedTarget( entity target, var damageInfo )
{
	...
	{
		int shieldRestoreAmount = target.GetArmorType() == ARMOR_TYPE_HEAVY ? 1250 : 500
		if( (soul.GetShieldHealth() + shieldRestoreAmount) > soul.GetShieldHealth()&&(soul.GetShieldHealthMax() ) <10000)
			soul.SetShieldHealthMax( soul.GetShieldHealthMax() + 125)
		soul.SetShieldHealth( min( soul.GetShieldHealth() + shieldRestoreAmount, soul.GetShieldHealthMax() ) )
		//if ( (soul.GetShieldHealth() + shieldRestoreAmount) > soul.GetShieldHealth())
		//	soul.SetShieldHealthMax( soul.GetShieldHealthMax() + 000)
		//soul.SetShieldHealth( min( soul.GetShieldHealth() + shieldRestoreAmount, soul.GetShieldHealthMax() ) )
	}
...
#endif
#if SERVER
void function punch_fighter( entity target, var damageInfo )
{
	entity attacker = DamageInfo_GetAttacker( damageInfo )
	entity soul = attacker.GetTitanSoul()
	if ( !IsValid( attacker ) )
		return
		int Amount = target.GetArmorType() == ARMOR_TYPE_HEAVY ? 500 : 20
		attacker.SetHealth( min( attacker.GetMaxHealth(),  attacker.GetHealth() + Amount) ) 
}
#endif