...
int function FireWeaponPlayerAndNPC( WeaponPrimaryAttackParams attackParams, bool playerFired, entity weapon )
{
	...
	if ( shouldCreateProjectile )
	{
		...
		bool hasMortarShotMod = weapon.HasMod( "power_shotfire" )
...
void function OnProjectileCollision_titanweapon_sticky_40mm( entity projectile, vector pos, vector normal, entity hitEnt, int hitbox, bool isCrit )
{
	...
	if( ( mods.contains( "power_shotfire" )&&mods.contains( "mortar_shots" ) || mods.contains( "tcp_no_gravity" ) ) && IsValid( owner ) )
	{
		if( ( mods.contains( "power_shotfire" )&& mods.contains( "mortar_shots" )&& mods.contains( "power_shotfire" )&& mods.contains( "mortar_shots" ) && !projectile.ProjectileGetMods().contains( "power_shotfire" ) &&!projectile.ProjectileGetMods().contains( "mortar_shots" )) || ( mods.contains( "tcp_no_gravity" ) && !projectile.ProjectileGetMods().contains( "tcp_no_gravity" ) ) )
		{
			RadiusDamage(
