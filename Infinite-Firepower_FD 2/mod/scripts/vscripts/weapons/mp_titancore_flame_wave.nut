		...
		if(weapon.HasMod("exfire")){
			if(IsValid(owner) && IsAlive(owner)){
				if(GetDoomedState(owner)){
					if(owner.GetHealth() > 1000 ){
						owner.TakeDamage( 1000 , owner ,null, {damageSourceId = eDamageSourceId.burn})  
					}
				}
				else{
					owner.TakeDamage( 2000 , owner ,null, {damageSourceId = eDamageSourceId.burn})  
				}
...
var function OnWeaponPrimaryAttack_titancore_flame_wave( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	entity inflictora = CreateDamageInflictorHelper( 10.0 )
	entity inflictorb = CreateDamageInflictorHelper( 10.0 )
	entity inflictorc = CreateDamageInflictorHelper( 10.0 )
	entity inflictord = CreateDamageInflictorHelper( 10.0 )
	entity inflictore = CreateDamageInflictorHelper( 10.0 )
	entity inflictorf = CreateDamageInflictorHelper( 10.0 )
	entity inflictorg = CreateDamageInflictorHelper( 10.0 )
	entity inflictorh = CreateDamageInflictorHelper( 10.0 )

	entity inflictor1 = CreateOncePerTickDamageInflictorHelper( 10.0 )
	entity inflictor2 = CreateOncePerTickDamageInflictorHelper( 10.0 )
	entity inflictor3 = CreateOncePerTickDamageInflictorHelper( 10.0 )
	entity inflictor4 = CreateOncePerTickDamageInflictorHelper( 10.0 )
	entity inflictor5 = CreateOncePerTickDamageInflictorHelper( 10.0 )
	entity inflictor6 = CreateOncePerTickDamageInflictorHelper( 10.0 )
	entity inflictor7 = CreateOncePerTickDamageInflictorHelper( 10.0 )
	entity inflictor8 = CreateOncePerTickDamageInflictorHelper( 10.0 )
	...
	if(weapon.HasMod("exfire")){
					thread BeginFlameWave( projectile, count, inflictor, attackParams.pos + offset, attackParams.dir )
					thread BeginFlameWave( projectile, count+1, inflictora, attackParams.pos + offset, <1.0,0,0.05> )
					thread BeginFlameWave( projectile, count+2, inflictorb, attackParams.pos + offset, < -1.0,0,0.05> )
					thread BeginFlameWave( projectile, count+3, inflictorc, attackParams.pos + offset, <0,1.0,0.05> )
					thread BeginFlameWave( projectile, count+4, inflictord, attackParams.pos + offset, <0,-1.0,0.05> )
					thread BeginFlameWave( projectile, count+5, inflictore, attackParams.pos + offset, <0.5,0.5,0.05> )
					thread BeginFlameWave( projectile, count+6, inflictorf, attackParams.pos + offset, < -0.5,0.5,0.05> )
					thread BeginFlameWave( projectile, count+7, inflictorg, attackParams.pos + offset, <0.5,-0.5,0.05> )
					thread BeginFlameWave( projectile, count+8, inflictorh, attackParams.pos + offset, < -0.5,-0.5,0.05> )

					//thread BeginScorchedEarth( projectile, count, inflictor, attackParams.pos + offset, attackParams.dir )
					thread BeginScorchedEarth( projectile, count+1, inflictor1, attackParams.pos + offset, <1.0,0,0.05> )
					thread BeginScorchedEarth( projectile, count+2, inflictor2, attackParams.pos + offset, < -1.0,0,0.05> )
					thread BeginScorchedEarth( projectile, count+3, inflictor3, attackParams.pos + offset, <0,1.0,0.05> )
					thread BeginScorchedEarth( projectile, count+4, inflictor4, attackParams.pos + offset, <0,-1.0,0.05> )
					thread BeginScorchedEarth( projectile, count+5, inflictor5, attackParams.pos + offset, <0.5,0.5,0.05> )
					thread BeginScorchedEarth( projectile, count+6, inflictor6, attackParams.pos + offset, < -0.5,0.5,0.05> )
					thread BeginScorchedEarth( projectile, count+7, inflictor7, attackParams.pos + offset, <0.5,-0.5,0.05> )
					thread BeginScorchedEarth( projectile, count+8, inflictor8, attackParams.pos + offset, < -0.5,-0.5,0.05> )
...
void function exfire(entity player)
{	
	...
	for(int timer = 1 ; timer < 30 ; timer++){
		...
		if((timer+2)/3 == 0){
			RadiusDamage(
				...
				(timer+10)*100,        // innerRadius
				(timer+10)*200,                // outerRadius
				SF_ENVEXPLOSION_NO_DAMAGEOWNER,            // flags
				0,                                        // distanceFromAttacker
				3000,                    // explosionForce
				DF_ELECTRICAL | DF_STOPS_TITAN_REGEN,eDamageSourceId.mp_titancore_flame_wave
