...
void function VoteMapInit(){
    ...
    AddClientCommandCallback("!Vote", CommandVote)
    AddClientCommandCallback("!V", CommandVote)
    AddClientCommandCallback("!v", CommandVote)
...
void function ChangeMapBeforeServer(){
    wait GAME_POSTMATCH_LENGTH - 2 // change 1 sec before server does
    string GAMEMODE = GetConVarString( "NSFD_Mode" )
    if(nextMap != "")
       {

        Chat_ServerBroadcast("正在改变地图++++==== >>>>")
        wait 0.5
        Chat_ServerBroadcast("正在改变地图++++++== >>>>")
        wait 0.5
        Chat_ServerBroadcast("正在改变地图++++++++ >>>>")
        wait 0.5
        GameRules_ChangeMap(nextMap, GAMEMODE)}
    else
    {
