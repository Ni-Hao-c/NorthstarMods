...
void function MpTitanAbilityLaserTrip_Init()
{
	...
	PrecacheParticleSystem( LASER_TRIP_ZAP_FX )
		...
		AddDamageCallbackSourceID( eDamageSourceId.mp_titancore_emp, LaserTripEMP_DamagedPlayerOrNPC )
	#endif
}
...
function DeployLaserPylon( entity projectile )
{
	...
	if( arcTrip )
	{
		thread EMPSonarThinkConstant( tower, owner )
		thread EMPSonarThinkConstant( tower, owner )
		thread EMPSonarThinkConstant( tower, owner )
		thread EMPSonarThinkConstant( tower, owner )
	}
...
void function LaserTripEMP_DamagedPlayerOrNPC( entity ent, var damageInfo )
{
	entity attacker = DamageInfo_GetAttacker( damageInfo )
	...
	if( !attacker.IsPlayer() && !attacker.IsNPC() )
		return
	if( !IsValid( attacker.GetOffhandWeapon( OFFHAND_TITAN_CENTER ) ) )
		return
	if( !attacker.GetOffhandWeapon( OFFHAND_TITAN_CENTER ).HasMod( "tcp_arc_trip" ) )
		return
	if( !IsValid( ent ) )
		return
