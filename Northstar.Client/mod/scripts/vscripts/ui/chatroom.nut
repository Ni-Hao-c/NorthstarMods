...
void function UpdateChatRoomButtons( InputDef data )
{
	...
	{
		bool progressionEnabled = Progression_GetPreference()
		string progressionTitle = progressionEnabled ? "#DISABLE_PROGRESSION" : "#ENABLE_PROGRESSION"

		foreach ( var menu in file.activeMenus )
		{
			if ( !NSIsVanilla() )
				SetFooterText( menu, index, Localize( progressionTitle ) )
			else if ( data.activateFunc == MuteRoom )
