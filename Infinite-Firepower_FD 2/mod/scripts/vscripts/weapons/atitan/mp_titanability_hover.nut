...
{	
	if(IsValid(player.GetOffhandWeapon(OFFHAND_TITAN_CENTER)))
	{	player.SetTitle("巡林者")
		player.GetTitanSoul().SetTitle("巡林者")
		entity Gweapon = player.GetOffhandWeapon(OFFHAND_TITAN_CENTER)
		if(Gweapon.HasMod("jumphover")){
			player.EndSignal( "OnDeath" )
			player.EndSignal( "TitanEjectionStarted" )

			thread AirborneThink( player, soundInfo )

			vector nowspeed = player.GetVelocity()
			vector lookforvec = Normalize(player.GetViewVector())
			float vx = lookforvec.x
			float vy = lookforvec.y
			player.SetVelocity(nowspeed + <450*vx , 450*vy ,1100>)

			OnThreadEnd(
				function() : (  player, soundInfo )
				{
					if ( IsValid( player ) )
					{
...
void function jumphit(entity player)
{
	wait 0.5
	if(IsValid(player) && IsAlive(player)){
		while ( player.IsOnGround() ){
			RadiusDamage(
			player.GetOrigin(),                                    // center
			player,                                    // attacker
			player,                                    // inflictor
			80,                    // damage
			...
			10000,                    // explosionForce
			DF_ELECTRICAL | DF_STOPS_TITAN_REGEN,    // scriptDamageFlags
			eDamageSourceId.damagedef_titan_fall )
