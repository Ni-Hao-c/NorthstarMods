...
{	
	...
	{	
		...
		if(weapon.HasMod("exsalvo")){
			entity ship = CreateNPC( "npc_gunship", weapon.GetWeaponOwner().GetTeam(), weapon.GetWeaponOwner().GetOrigin()+<0,0,4200>,  weapon.GetWeaponOwner().GetAngles() )
			DispatchSpawn(ship)
			ship.Freeze()
			ship.SetTakeDamageType(DAMAGE_NO)
			vector atkpos = ship.GetOrigin()
			thread shiplife(ship)	
			weapon.GetWeaponOwner().s.atkpos <- atkpos
...
var function OnWeaponPrimaryAttack_titanweapon_salvocore_rockets( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	foreach ( v in verticalOffsets )
	{
		...
		if(weapon.HasMod("exsalvo")){
			vector atkpos = expect vector(weapon.GetWeaponOwner().s.atkpos)
			entity missile = weapon.FireWeaponMissile( atkpos-<0,0,250>, attackDir-<0,0,10>, RandomFloatRange( 0.8, 1.1 ), (damageTypes.projectileImpact | DF_DOOM_FATALITY), damageTypes.explosive, doPopup, shouldPredict )
			
			if ( missile )
			{
				missile.InitMissileForRandomDriftFromWeaponSettings( attackPos, attackDir )
				missile.SetOrigin(atkpos-<0,0,250>)
				...
				//missile.SetModel($"models/weapons/bullets/projectile_rocket_large.mdl")
				missile.kv.modelscale = 5
				missile.SetHomingSpeeds( 75, 0 )
...
void function shiplife(entity ship)
{
	wait 13
